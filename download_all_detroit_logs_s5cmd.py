#!/usr/bin/env python3
"""
Download All Detroit Logs (s5cmd version)

Download all Detroit (DTW) logs from train, val, and test splits using s5cmd
for faster S3 operations. Organize them in a structured directory format.
"""
...
class DetroitLogDownloader:
    ...
    def find_detroit_logs_in_split(self, split):
        """Find all Detroit logs in a specific split using s5cmd."""
        
        print(f"\nSearching for Detroit logs in {split} split...")
        
        try:
            # Get list of all logs using s5cmd
            ...
            # Extract log IDs from s5cmd output
            log_ids = [] 
            ...
            for i, log_id in enumerate(tqdm(log_ids, desc=f"Checking {split} logs")):
    ...
    def download_log_data(self, split, log_id):
        """Download essential data for a single Detroit log using s5cmd."""
        ...
        # Download each file using s5cmd
        downloaded = 0
    ...
    def download_all_detroit_data(self, max_workers=5):
        """Download data for all found Detroit logs using parallel s5cmd operations."""
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Use tqdm for progress bar
            for future in tqdm(as_completed(future_to_log), total=len(all_downloads), desc="Downloading logs"):
...
def main():
    ...
    print("Detroit Logs Downloader - Argoverse 2 (s5cmd version)")
    ...
    try:
        ...
        print("Error: s5cmd is not installed or not in PATH")
        print("Please install s5cmd first:")
        print("  macOS: brew install s5cmd")
        print("  Linux: curl -L https://github.com/peak/s5cmd/releases/download/v2.2.2/s5cmd_2.2.2_Linux-64bit.tar.gz | tar -xz")
        print("         sudo mv s5cmd /usr/local/bin/")
        return
    ...
    # Download all data
    print(f"\nStarting download...")
    downloader.download_all_detroit_data(max_workers=5)  # Increased workers for s5cmd
